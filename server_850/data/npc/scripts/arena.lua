...
function creatureSayCallback(cid, type, msg)
    ...
    if msgcontains(msg, 'fight') then
            selfSay('So you agree to the rules and want to participate in the challenge? Which difficulty do you choose, {Warlord}?', cid)
    ...
    elseif talkState[talkUser] == 1 then
        if msgcontains(msg, 'dificulties') then
            ...
            elseif msgcontains(msg, 'prices') then
                selfSay('A great warlord cost 10000 golds.', cid)
                talkState[talkUser] = 0
            end
